...
class AppStore extends Component {
  state = {
    selectedTabItem: tabsList[0],
    searchInputValue: '',
  }

  onChangeTabItem = tabId => {
    const updatedTabItem = tabsList.filter(eachTab => eachTab.tabId === tabId)

    this.setState({selectedTabItem: updatedTabItem[0]})
  }
  ...
  render() {
    const {selectedTabItem, searchInputValue} = this.state

    const categoryApps = appsList.filter(
      eachApp => eachApp.category === selectedTabItem.tabId,
    )

    const searchFilteredApps = categoryApps.filter(eachApp =>
        ...
        <ul className="tabs-container">
          {tabsList.map(eachTab => (
            <TabItem
              ...
              isTabActive={selectedTabItem.tabId === eachTab.tabId}
            />
          ))}
        </ul>
